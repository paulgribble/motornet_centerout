...
class CentreOutFF(mn.environment.Environment):
    ...
    def reset(self, *,
              ...
              K: float = 1,
              B: float = -1,
              go_cue_range: Union[list, tuple, np.ndarray] = (
                  0.1, 0.6),  # range of times of randomized go-cue onset
        ...
        joint_state: th.Tensor | np.ndarray | None = options.get(
            'joint_state', None)
        ...
        if (condition == 'train'):  # train net to reach to random targets
            ...
            goal = self.joint2cartesian(
                self.effector.draw_random_uniform_states(batch_size)).chunk(2, dim=-1)[0]
            self.goal = goal if self.differentiable else self.detach(goal)

            # specify go cue time
            go_cue_time = np.random.uniform(
                self.go_cue_range[0], self.go_cue_range[1], batch_size)
            ...
            start_jpv = th.from_numpy(np.concatenate([start_position, np.zeros_like(
                start_position)], axis=1))  # joint position and velocity
            start_cpv = self.joint2cartesian(start_jpv).numpy()
            end_cp = reaching_distance * \
                np.stack([np.cos(angle), np.sin(angle)], axis=-1)

            goal_states = start_cpv + \
                np.concatenate([end_cp, np.zeros_like(end_cp)], axis=-1)
        ...
        self.obs_buffer["proprioception"] = [
            self.get_proprioception()] * len(self.obs_buffer["proprioception"])
        self.obs_buffer["vision"] = [self.get_vision()] * \
            len(self.obs_buffer["vision"])
    ...
    def step(self, action, deterministic: bool = False, **kwargs):
        ...
        if self.is_channel:  # force channel probe trial
            ...
            projection = th.sum(line_vector * xy, axis=-1) / \
                th.sum(line_vector * line_vector, axis=-1)
            projection = line_vector * projection[:, None]

            err = xy - projection

            projection = th.sum(line_vector * vel, axis=-1) / \
                th.sum(line_vector * line_vector, axis=-1)
            projection = line_vector * projection[:, None]
            ...
            FF_matvel = th.tensor(
                [[0, 1], [-1, 0]], dtype=th.float32).to("cpu")
        ...
        # specify go cue time
        mask = self.elapsed >= (
            self.go_cue_time + (self.vision_delay-1) * self.dt)
        self.go_cue[mask] = 1
